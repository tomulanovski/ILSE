#!/usr/bin/env python3
"""
Optuna hyperparameter search for LINEAR MLP on STS tasks with precomputed embeddings.
Linear version: use_linear=True removes all ReLU activations.
Fixed architecture: 1 hidden layer, last layer input, hidden_dim=256.
"""
import optuna
...
from experiments.utils.model_definitions.gnn.gnn_datasets import PrecomputedSTSTensorDataset
from experiments.utils.model_definitions.gnn.gnn_models import MLPEncoder, PairCosineSimScore
...
def train_and_eval_model(args):
    """Train and evaluate LINEAR MLP model with precomputed embeddings."""
    print(f"[DEBUG] Starting train_and_eval_model for LINEAR MLP task: {args.task}")
    ...
    in_dim = sample_a.shape[0]
    print(f"[DEBUG] Input dimension: {in_dim}")

    # Build LINEAR MLP model (use_linear=True removes all ReLU activations)
    mlp_encoder = MLPEncoder(
        in_dim=in_dim,
        hidden_dim=256,  # Fixed: hidden_dim=256
        num_layers=1,    # Fixed: 1 hidden layer
...
if __name__ == "__main__":
    ...
    # NOTE: mlp_input fixed to "last", specific_layer removed for linear version
    parser.add_argument("--embeddings_dir", type=str, required=True)
    parser.add_argument("--storage_url", type=str, required=True)
    parser.add_argument("--n_trials", type=int, default=1)
    cmd_args = parser.parse_args()

    print(f"[INFO] LINEAR MLP will use last layer only with 1 hidden layer (linear, no ReLU)")
    ...
    def objective_with_args(trial):
        ...
        args = Namespace(
            ...
            encoder="mlp",
            # Fixed architecture
            mlp_input="last",    # Fixed: last layer only
            mlp_hidden_dim=256,  # Fixed: hidden_dim=256
            mlp_layers=1,        # Fixed: 1 hidden layer
