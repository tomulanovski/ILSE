#!/usr/bin/env python3
"""
Optuna hyperparameter search for DeepSet on STS tasks with precomputed embeddings.
Much faster than recomputing embeddings on every trial.

DeepSet architecture: φ(layers) → POOL → ρ(pooled) → output
- φ: shared MLP applied to each layer independently
- POOL: permutation-invariant aggregation (mean or sum)
- ρ: MLP applied to pooled result
"""
import optuna
...
def train_and_eval_model(args):
    """Train and evaluate DeepSet model with precomputed embeddings."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--deepset_hidden_dim", type=int, default=256, help="Hidden dimension for DeepSet MLPs")
    ...
    def objective_with_args(trial):
        ...
        args = Namespace(
            ...
            encoder="deepset",
            deepset_hidden_dim=cmd_args.deepset_hidden_dim,
            deepset_pre_pooling_layers=trial.suggest_int("deepset_pre_pooling_layers", 0, 2),
