#!/usr/bin/env python3
"""
Optuna hyperparameter search for MLP on STS tasks with precomputed embeddings.
"""
import optuna
...
def train_and_eval_model(args):
    """Train and evaluate MLP model with precomputed embeddings."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--specific_layer", type=int, default=None,
                        help="Use specific layer (e.g., 15). If not set, uses 'last' layer only")
    parser.add_argument("--embeddings_dir", type=str, required=True)
    parser.add_argument("--storage_url", type=str, required=True)
    parser.add_argument("--n_trials", type=int, default=1)
    cmd_args = parser.parse_args()

    # Determine MLP input mode
    if cmd_args.specific_layer is not None:
        mlp_input_mode = "last"  # Will be handled in dataset loading with specific layer
        print(f"[INFO] MLP will use specific layer {cmd_args.specific_layer}")
        # TODO: Implement layer_N mode in PrecomputedSTSTensorDataset
    else:
        mlp_input_mode = "last"  # Default to last layer only
        print(f"[INFO] MLP will use last layer only")
    ...
    def objective_with_args(trial):
        args = Namespace(
            ...
            encoder="mlp",
            mlp_input=mlp_input_mode,  # Fixed to "last" or specific layer
            mlp_hidden_dim=256,
            mlp_layers=trial.suggest_int("mlp_layers", 1, 3),  # Minimum 1 layer (was 0, causes empty model)
