#!/usr/bin/env python3
"""
Optuna hyperparameter search for DWAtt (Depth-Wise Attention) encoder on STS tasks
...
from experiments.utils.model_definitions.gnn.gnn_models import DWAttEncoder, PairCosineSimScore
...
def train_and_eval_model(args):
    """Train and evaluate DWAtt model with precomputed embeddings."""
    ...
    # Build DWAtt model (architecture follows paper Table 4)
    # hidden_dim: None = paper-faithful (operates in layer_dim), 256 = controlled comparison with GIN
    dwatt_hidden_dim = getattr(args, 'dwatt_hidden_dim', None)
    dwatt_encoder = DWAttEncoder(
        num_layers=L,
        layer_dim=D,
        hidden_dim=dwatt_hidden_dim,  # None = paper-faithful, 256 = fair comparison with GIN
        bottleneck_ratio=args.dwatt_bottleneck_ratio,
        pos_embed_dim=getattr(args, 'dwatt_pos_embed_dim', 24),  # Paper: d_pos = 24
        dropout=args.dropout
    )
    model = PairCosineSimScore(dwatt_encoder).to(device)

    model_param_count = sum(p.numel() for p in model.parameters())
    print(f"DWAtt model parameters: {model_param_count:,}")
...
if __name__ == "__main__":
    ...
    def objective_with_args(trial):
        # DWAtt architecture - FIXED per paper (Table 4):
        # - d_pos = 24, bottleneck_ratio = 0.5
        # Training hyperparameters - same as other baselines for fair comparison
        args = Namespace(
            task=cmd_args.task,
            ...
            encoder="dwatt",
            # DWAtt architecture (per paper Table 4)
            # hidden_dim options:
            #   - None (default): Paper-faithful, operates in layer_dim (e.g., 1024)
            #   - 256: Controlled comparison with GIN (projects to 256 first)
            dwatt_hidden_dim=None,             # Paper-faithful (no input projection)
            dwatt_bottleneck_ratio=0.5,        # Paper: γ = 0.5
            dwatt_pos_embed_dim=24,            # Paper: d_pos = 24
            # Training hyperparameters (same as other baselines)
