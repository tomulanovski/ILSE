#!/usr/bin/env python3
"""
Optuna hyperparameter search for LINEAR DeepSet on STS tasks with precomputed embeddings.
Linear version: use_linear=True removes all ReLU activations.
Fixed architecture: hidden_dim=256, 2 variants (pre1_post0 and pre0_post1) as hyperparameter.

DeepSet architecture: φ(layers) → POOL → ρ(pooled) → output
- φ: shared MLP applied to each layer independently (pre-pooling)
- POOL: permutation-invariant aggregation (mean or sum)
- ρ: MLP applied to pooled result (post-pooling)
"""
import optuna
...
from experiments.utils.model_definitions.gnn.gnn_models import DeepSetEncoder, PairCosineSimScore
...
def train_and_eval_model(args):
    """Train and evaluate LINEAR DeepSet model with precomputed embeddings."""
    print(f"[DEBUG] Starting train_and_eval_model for LINEAR DeepSet task: {args.task}")
    print(f"[DEBUG] Variant: pre={args.deepset_pre_pooling_layers}, post={args.deepset_post_pooling_layers}")
    ...
    # Build LINEAR DeepSet model (use_linear=True removes all ReLU activations)
    deepset_encoder = DeepSetEncoder(
        ...
        use_linear=True  # NEW: Enable linear mode (no ReLU)
    )
    model = PairCosineSimScore(deepset_encoder).to(device)

    model_param_count = sum(p.numel() for p in model.parameters())
    print(f"[DEBUG] Model parameter count: {model_param_count}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_trials", type=int, default=1, help="Number of trials (default: 1, recommended total: 25)")
    # NOTE: deepset_hidden_dim fixed to 256, variant is a hyperparameter (not command-line arg)
    cmd_args = parser.parse_args()

    print(f"[INFO] LINEAR DeepSet will search 2 variants (pre1_post0 and pre0_post1) within the same study")

    def objective_with_args(trial):
        print(f"[DEBUG] Objective function called for trial {trial.number}")

        # NEW: Variant as hyperparameter (not command-line arg)
        variant = trial.suggest_categorical("variant", ["pre1_post0", "pre0_post1"])

        # Set architecture based on variant
        if variant == "pre1_post0":
            deepset_pre_pooling_layers = 1
            deepset_post_pooling_layers = 0
        else:  # pre0_post1
            deepset_pre_pooling_layers = 0
            deepset_post_pooling_layers = 1

        print(f"[DEBUG] Variant: {variant} (pre={deepset_pre_pooling_layers}, post={deepset_post_pooling_layers})")
        ...
        args = Namespace(
            ...
            encoder="deepset",
            # Fixed architecture
            deepset_hidden_dim=256,  # Fixed: hidden_dim=256
            # Variant-determined architecture
            deepset_pre_pooling_layers=deepset_pre_pooling_layers,
            deepset_post_pooling_layers=deepset_post_pooling_layers,
