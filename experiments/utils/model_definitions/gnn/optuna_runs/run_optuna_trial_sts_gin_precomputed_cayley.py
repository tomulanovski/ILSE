#!/usr/bin/env python3
"""
Optuna hyperparameter search for GIN on STS tasks with precomputed embeddings (Cayley graph).
Uses pool_real_nodes_only, learnable epsilon, and mean/sum pooling.
Much faster than recomputing embeddings on every trial.
"""
import optuna
...
def train_and_eval_model(args):
    """Train and evaluate GIN model with precomputed embeddings."""
    print(f"[DEBUG] Starting train_and_eval_model for task: {args.task}")
    ...
    train_dataset = PrecomputedSTSGraphDataset(
        ...
        keep_embedding_layer=args.pool_real_nodes_only  # Keep embedding layer
    )
    ...
    # Build model with NEW pooling options and learnable epsilon
...
if __name__ == "__main__":
    ...
    parser.add_argument("--graph_type", type=str, required=True,
                        ...
                        help="Graph type for this study (fixed per study)")
    ...
    # Pooling options: mean and sum only
    pooling_options = ["mean", "sum"]

    def objective_with_args(trial):
        print(f"[DEBUG] Objective function called for trial {trial.number}")
        print(f"[DEBUG] Graph type: {cmd_args.graph_type}, Pooling options: {pooling_options}")

        # Cayley: pool_real_nodes_only is always True (exclude virtual nodes from pooling)
        pool_real_nodes_only = True

        # NEW: Add train_eps to search space (learnable epsilon in GIN aggregation)
        ...
        args = Namespace(
            ...
            train_eps=train_eps,            graph_type=cmd_args.graph_type,  # Fixed per study
            epochs=25,
        ...
        print(f"[DEBUG] Hyperparameters: pool_real_nodes_only=True (fixed), train_eps={train_eps}, node_to_choose={args.node_to_choose}")
        print(f"[DEBUG] Calling train_and_eval_model")
