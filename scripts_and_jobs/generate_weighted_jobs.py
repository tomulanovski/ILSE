#!/usr/bin/env python3
"""
Generate SLURM jobs for Weighted baseline Optuna experiments.
...
Total: 3 models × 7 tasks = 21 job files (each spawns 50 array tasks)
"""
...
def get_model_dir_name(model_family, model_size):
    """Get directory name for weighted jobs."""
    family_lower = model_family.lower()
    if family_lower == "llama3":
        return f"{family_lower}_weighted_optuna"
    else:
        return f"{family_lower}{model_size}_weighted_optuna"


def create_weighted_job(model_family, model_size, task, time_limit, memory):
    """Create a single Weighted Optuna SLURM job script (array job with 50 trials)."""

    job_name = f"weighted_{task}_{model_family}_{model_size}"
    study_name = f"weighted_{task}_{model_family}_{model_size}"
    log_dir = f"{REPO_DIR}/job_logs/logs_weighted_{task}_{model_family}_{model_size}"
...
time python3 -m experiments.utils.model_definitions.gnn.optuna_runs.run_optuna_trial_weighted \\
...
def main():
    print("Generating Weighted baseline Optuna SLURM jobs...\n")
    ...
    for model_family, model_size, time_limit, memory in MODELS:
        ...
        for task in TASKS:
            job_script = create_weighted_job(
    ...
    print(f"\n{'='*70}")
    print(f"Total Weighted Optuna job files created: {job_count}")
    ...
    print("To submit all Weighted jobs, run:")
    ...
    print(f"  squeue -u $USER | grep tom_weighted")
    print()
    print("\nNote: Weighted baseline has only ~24-32 learnable parameters")
    print("(one weight per layer), making it the minimal learnable baseline.")
