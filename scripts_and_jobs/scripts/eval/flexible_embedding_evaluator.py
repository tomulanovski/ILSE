...
Supports: single layer, mean pooling, GNN-encoded, MLP-encoded, Weighted embeddings.
"""
import argparse
import json
import numpy as np
...
from scripts_and_jobs.scripts.eval.precomputed_evaluator import (
    load_gnn_encoder_from_checkpoint,
    load_mlp_encoder_from_checkpoint,
    load_weighted_encoder_from_checkpoint,
    encode_with_gnn,
    encode_with_mlp,
    encode_with_weighted
)
from scripts_and_jobs.scripts.eval.mteb_embedding_evaluator import evaluate_embeddings
...
def extract_embeddings(
    ...
    Args:
        embedding_type: 'layer', 'mean', 'gin', 'mlp', or 'weighted'
        layerwise_embeddings: (N, L, D) layer-wise embeddings
        layer_idx: Layer index for 'layer' type
        model_path: Path to model checkpoint for 'gin'/'mlp'/'weighted'
    ...
    elif embedding_type == "weighted":
        if model_path is None and encoder is None:
            raise ValueError("--model_path required for embedding_type='weighted'")

        if encoder is None:
            if device is None:
                device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

            encoder, saved_args = load_weighted_encoder_from_checkpoint(model_path, L, D, device)

        print(f"  Encoding with Weighted (learned layer weighting)...")
        return encode_with_weighted(encoder, layerwise_embeddings, batch_size, device)
...
def evaluate_task(
    ...
    Args:
        ...
        embedding_type: 'layer', 'mean', 'gin', 'mlp', or 'weighted'
        layer_idx: Layer index (for 'layer' type)
        model_path: Model checkpoint path (for 'gin'/'mlp'/'weighted')
    ...
    if embedding_type in ("gin", "mlp", "weighted"):
...
def main():
    ...
    parser.add_argument("--embedding_type", type=str, required=True,
                        choices=["layer", "mean", "gin", "mlp", "weighted"],
    ...
    parser.add_argument("--model_path", type=str, default=None,
                        help="Model checkpoint path (required for embedding_type='gin'/'mlp'/'weighted')")
    ...
    if args.embedding_type in ("gin", "mlp", "weighted") and args.model_path is None:
        parser.error("--model_path required for embedding_type='gin'/'mlp'/'weighted'")
    ...
    # Save results in MTEB-compatible format for summarize_with_layers.py
    output_path = Path(args.output_results)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    # Convert to MTEB FullClassificationMetrics format
    # Include all MTEB metrics + per-experiment scores
    mteb_format = {
        # Main metrics (averaged across experiments)
        ...
        "ap": results.get("ap"),
        "ap_weighted": results.get("ap_weighted"),

        # Per-experiment scores (full MTEB output)
        "scores_per_experiment": results.get("scores_per_experiment", []),

        # Task metadata (wrapped in "test" for backward compatibility with summarizer)
        "test": {
            "accuracy": results.get("accuracy", 0.0),
            "f1": results.get("f1", 0.0),
            "f1_weighted": results.get("f1_weighted", 0.0),
    ...
    with open(output_path, 'w') as f:
        json.dump(mteb_format, f, indent=2)
